...
public:
 ...
 /*   openni::VideoFrameRef *downloadToNewFrame()
    {
        openni::VideoFrameRef *f = new openni::VideoFrameRef(width, height, bytes_per_pixel);
        downloadToBuffer(f->getData());
        flipYBuffer(f->getData());
...
private:
    ...
    std::map<std::string,openni::VideoFrameRef*> frames;
    Texture<F8C4> rgb;
    Texture<F32C1> ir;
    int win_width;
    int win_height;
public:
    ...
    virtual void onOpenGLBindingsChanged(OpenGLBindings *b);
    bool render();
    void addFrame(std::string id,openni::VideoFrameRef* frame);
