    ...
    void process(FrameData &frameData) {
        ...
        for( int i=0; i<(int)frameData.images.size(); i++) {
            ...
            foreach( object, *frameData.objects ) {
                ...
                if(object->model->name == "cup")
                {
                    if(object->score <10)
                        continue;
                }

                if(object->model->name == "juice")
                {
                    if(object->score <10)
                        continue;
                }
            ...
            Pt<8> times;
            times[0] = frameData.times["UNDISTORTED_IMAGE"];
            times[1] = frameData.times["SIFT"];
            times[2] = frameData.times["MATCH_SIFT"];
            ...
            //    cout << "times[" << tt << "]:" << times[tt]*1000. << endl;
